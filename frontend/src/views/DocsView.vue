  ...
  <div class="container docs-shell">
    <aside class="card docs-tree">
      ...
      <ul v-else class="docs-list">
        <li v-for="node in displayNodes" :key="node.id">
          <button
            type="button"
            class="docs-link"
            :class="{ active: node.id === selectedId, folder: node.type === 'FOLDER', hidden: node.hidden }"
          ...
          >
            ...
            <span
              ...
              v-html="node.title"
            ></span>
            <span v-else class="docs-title">{{ node.title }}</span>
          </button>
          <p v-if="searchQuery && node.content" class="docs-snippet" v-html="node.content"></p>
        </li>
      </ul>
    </aside>
