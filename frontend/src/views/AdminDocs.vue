<template>
  <AdminShell title="Documents" subtitle="Manage the document tree">
    ...
    <section class="admin-docs">
      <aside
        class="card docs-tree"
        :class="{ 'drag-root': rootDropActive }"
      ...
      >
        ...
        <ul v-else class="docs-list">
          <li v-for="node in flatNodes" :key="node.id">
            <button
              type="button"
              class="docs-link"
              :class="{
                ...
                hidden: node.hidden,
                'drop-before': dragOverId === node.id && dragOverPosition === 'before',
                'drop-after': dragOverId === node.id && dragOverPosition === 'after'
              }"
            ...
            >
              <span class="docs-icon">{{ node.type === 'FOLDER' ? 'DIR' : 'DOC' }}</span>
              <span class="docs-title">{{ node.title }}</span>
            </button>
          </li>
        </ul>
      </aside>
      ...
      <section class="card docs-editor">
        <div class="docs-editor-header">
          <div>
            ...
            <p v-if="selectedNode" class="docs-muted">Selected: {{ selectedNode.title }}</p>
          </div>
          <div class="docs-editor-actions" v-if="selectedNode">
            <n-button type="default" class="btn btn-soft" @click="moveUp" :disabled="saving || !canMoveUp">
              Move Up
            </n-button>
            <n-button type="default" class="btn btn-soft" @click="moveDown" :disabled="saving || !canMoveDown">
              Move Down
            </n-button>
        ...
        <n-form label-placement="top" v-if="isCreating || selectedNode">
          ...
          <ul v-else class="docs-revision-list">
            <li v-for="rev in revisions" :key="rev.id" class="docs-revision-item">
              ...
              <div class="docs-revision-actions">
                <n-button size="small" class="btn btn-soft" @click="selectRevision(rev.id)" :disabled="saving">
                  View Diff
                </n-button>
                <n-button size="small" class="btn btn-soft" @click="restoreRevision(rev.id)" :disabled="saving">
                  Restore
                </n-button>
              </div>
            </li>
...
const { result: revisionsResult, loading: revisionsLoading, refetch: refetchRevisions } = useQuery(
  gql`
...
const onDragStart = (node: DocumentNode) => {
...
const onDragOver = (event: DragEvent, node: DocumentNode) => {
  if (!draggingId.value || draggingId.value === node.id) return
  dragOverId.value = node.id
  dragOverPosition.value = shouldPlaceAfter(event) ? 'after' : 'before'
}

const onDragLeave = (node: DocumentNode) => {
...
const onDropNode = async (event: DragEvent, target: DocumentNode) => {
  event.stopPropagation()
  if (!draggingId.value || draggingId.value === target.id) return
  const dragged = flatNodes.value.find((node) => node.id === draggingId.value)
  if (!dragged) return
  const sameParent = (dragged.parentId ?? null) === (target.parentId ?? null)
  const placeAfter = shouldPlaceAfter(event)

  if (sameParent) {
    await reorderWithinParent(dragged, target, placeAfter)
  } else if (target.type === 'FOLDER' && dragged.parentId !== target.id) {
...
const onDropRoot = async () => {
  ...
  const dragged = flatNodes.value.find((node) => node.id === draggingId.value)
  if (!dragged) return
  await moveToParent(dragged, null)
  draggingId.value = null
  dragOverId.value = null
  dragOverPosition.value = null
  rootDropActive.value = false
...
const moveToParent = async (node: DocumentNode, parentId: number | null) => {
  const siblingOrders = flatNodes.value
...
const moveToParentAt = async (node: DocumentNode, parentId: number | null, target: DocumentNode, placeAfter: boolean) => {
  const siblings = flatNodes.value
    .filter((item) => (item.parentId ?? null) === (parentId ?? null) && item.id !== node.id)
  ...
  try {
    if (isCreating.value) {
      ...
      const needsUpdate =
        ...
        (form.type === 'DOC' ? form.content : null) !== (current.content ?? null) ||
        Number(form.sortOrder) !== Number(current.sortOrder ?? 0) ||
        Boolean(form.hidden) !== Boolean(current.hidden) ||
...
.docs-tree.drag-root {
  ...
  box-shadow: 0 0 0 2px var(--theme-a20);
}
...
.docs-link.drop-before::before,
.docs-link.drop-after::after {
  content: '';
  position: absolute;
  left: 10px;
  right: 10px;
