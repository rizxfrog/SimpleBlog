    ...
    <section class="editor-layout">
      <aside class="card tree-panel">
        ...
        <n-tree
          ...
          default-expand-all
          expand-on-click
          :on-update:selected-keys="onSelectTree"
        />
        ...
        <template v-if="selectedNode">
          ...
          <div v-if="selectedNode.nodeType === 'DOC' && selectedNode.docId" class="commit-panel">
            ...
            <n-space vertical :size="10">
              ...
              <n-space>
                <n-select
                  v-model:value="mergeSourceRef"
                  :options="mergeSourceOptions"
                  placeholder="选择来源分支"
                  style="width: 260px"
                />
                <n-button :disabled="!mergeSourceRef || !latestCommit" @click="mergeFromBranch">
                  合并到当前分支
                </n-button>
              </n-space>
              ...
              />
              <n-input
                v-model:value="commitForm.contentMd"
                type="textarea"
                :autosize="{ minRows: 14, maxRows: 24 }"
                placeholder="Markdown 内容"
              />
              <n-button type="primary" :loading="saving" @click="commitCurrent">
                提交到 {{ selectedRef || 'refs/heads/main' }}
              </n-button>
            </n-space>
          </div>
        </template>
...
const parentOptions = computed(() => {
  return flatNodes.value
    .filter(item => item.nodeType === 'FOLDER' && item.id !== selectedNodeId.value)
...
const mergeSourceOptions = computed(() => {
  return branchRefs.value
    .filter(item => item.refName !== selectedRef.value)
    .map(item => ({ label: item.refName, value: item.refName }));
});
