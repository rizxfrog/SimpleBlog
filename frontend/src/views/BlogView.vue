			...
			<section class="blog-article">
				<div class="article-hero">
					...
					<div class="article-actions">
						<button class="article-action vote-btn" type="button" :class="{ 'is-active': post.userVote === 1 }" aria-label="Like" @click="voteBlog(1)">
							<Icon class="vote-icon" :icon="post.userVote === 1 ? 'mdi:thumb-up' : 'mdi:thumb-up-outline'" />
							<span>{{ post.likes || 0 }}</span>
						</button>
						<button class="article-action vote-btn" type="button" :class="{ 'is-active': post.userVote === -1 }" aria-label="Dislike" @click="voteBlog(-1)">
							<Icon class="vote-icon" :icon="post.userVote === -1 ? 'mdi:thumb-down' : 'mdi:thumb-down-outline'" />
			...
			<div v-if="comments.length" class="comment-list">
				<div v-for="comment in comments" :key="comment.id" class="comment-item">
					...
					<div class="comment-actions">
						<button class="comment-action" type="button" @click="voteComment(comment.id, 1)">👍 {{ comment.upvotes || 0 }}</button>
						<button class="comment-action" type="button" @click="voteComment(comment.id, -1)">👎 {{ comment.downvotes || 0 }}</button>
						<button class="comment-action" type="button" @click="toggleReplyForm(comment.id)">回复</button>
					</div>
					<div v-if="showReplyForm[comment.id]" class="comment-reply-form">
						<textarea v-model="replyContent[comment.id]" rows="3" placeholder="写下你的回复..."></textarea>
					...
					<div v-if="comment.replies?.length" class="comment-replies">
						<div v-for="reply in comment.replies" :key="reply.id" class="comment-item reply-item">
							...
							<p>{{ reply.content }}</p>
							<div class="comment-actions">
								<button class="comment-action" type="button" @click="voteComment(reply.id, 1)">👍 {{ reply.upvotes || 0 }}</button>
								<button class="comment-action" type="button" @click="voteComment(reply.id, -1)">👎 {{ reply.downvotes || 0 }}</button>
							</div>
						</div>
					</div>
				</div>
